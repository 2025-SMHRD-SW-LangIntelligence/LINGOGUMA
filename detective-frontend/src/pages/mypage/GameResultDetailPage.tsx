// src/pages/mypage/GameResultDetailPage.tsx
import { useEffect, useMemo, useState } from "react";
import { useLocation, useNavigate } from "react-router-dom";
import { Radar } from "react-chartjs-2";
import {
  Chart as ChartJS,
  RadialLinearScale,
  PointElement,
  LineElement,
  Filler,
  Tooltip,
  Legend,
} from "chart.js";
import { api } from "@/shared/api/client";
import "@/shared/styles/MyPage.css";

ChartJS.register(
  RadialLinearScale,
  PointElement,
  LineElement,
  Filler,
  Tooltip,
  Legend
);

type SkillsJson = {
  logic?: number | string;
  creativity?: number | string;
  focus?: number | string;
  diversity?: number | string;
  depth?: number | string;
};

export default function GameResultDetailPage() {
  const location = useLocation();
  const navigate = useNavigate();

  const result = location.state as {
    scenIdx: number;
    correct: boolean;
    answerJson: any;
    skillsJson: SkillsJson;
  } | null;

  const [culpritName, setCulpritName] = useState<string>("");

  if (!result) return <p className="form-error">결과 데이터가 없습니다.</p>;

  const { scenIdx, answerJson, skillsJson, correct } = result;

  // ✅ 숫자 안전 변환
  const s = {
    logic: Number(skillsJson?.logic ?? 0),
    creativity: Number(skillsJson?.creativity ?? 0),
    focus: Number(skillsJson?.focus ?? 0),
    diversity: Number(skillsJson?.diversity ?? 0),
    depth: Number(skillsJson?.depth ?? 0),
  };

  // ✅ 평균 점수
  const avgScore = useMemo(
    () =>
      Math.round(
        (s.logic + s.creativity + s.focus + s.diversity + s.depth) / 5
      ),
    [s]
  );

  // ✅ 용의자 ID → 이름 매핑
  useEffect(() => {
    // answerJson.culprit 이 이미 이름일 수도 있으니 먼저 사용
    const raw = answerJson?.culprit;
    if (raw && typeof raw === "object" && typeof raw.name === "string") {
      setCulpritName(raw.name);
      return;
    }
    if (
      typeof raw === "string" &&
      raw.trim() &&
      !raw.startsWith("suspect_") &&
      raw.length <= 20
    ) {
      // 짧은 문자열이고 suspect_ 패턴이 아니면 이름으로 간주
      setCulpritName(raw.trim());
      return;
    }

    // ID일 가능성이 높으면 시나리오 characters에서 매핑
    const culpritId = String(raw ?? "").trim();
    if (!culpritId) {
      setCulpritName("미입력");
      return;
    }

    (async () => {
      try {
        const res = await api.get(`/scenarios/${scenIdx}`);
        let content: any = res.data?.contentJson;
        if (typeof content === "string") {
          try {
            content = JSON.parse(content);
          } catch {
            content = {};
          }
        }
        const chars: any[] = Array.isArray(content?.characters)
          ? content.characters
          : [];
        const nameMap: Record<string, string> = {};
        for (const c of chars) {
          const id = String(c?.id ?? "");
          const nm = String(c?.name ?? "");
          if (id && nm) nameMap[id] = nm;
        }
        setCulpritName(nameMap[culpritId] || culpritId || "미입력");
      } catch {
        setCulpritName(culpritId || "미입력");
      }
    })();
  }, [answerJson, scenIdx]);

  // ✅ Chart.js 데이터/옵션
  const data = {
    labels: ["논리력", "창의력", "집중력", "다양성", "깊이"],
    datasets: [
      {
        label: "플레이어 능력치",
        data: [s.logic, s.creativity, s.focus, s.diversity, s.depth],
        backgroundColor: "rgba(34, 202, 236, 0.25)",
        borderColor: "#22caec",
        borderWidth: 2,
        pointBackgroundColor: "#22caec",
      },
    ],
  };

  const options = {
    responsive: true,
    maintainAspectRatio: false,
    scales: {
      r: {
        min: 0,
        max: 100,
        grid: { color: "rgba(255,255,255,0.25)" },
        angleLines: { color: "rgba(255,255,255,0.3)" },
        ticks: {
          stepSize: 20,
          color: "#eee",
          backdropColor: "rgba(255,255,255,0.25)",
          backdropPadding: 2,
        },
        pointLabels: {
          color: "#eee",
          font: { size: 13, weight: "700" },
        },
      },
    },
    plugins: {
      legend: { display: false },
      tooltip: {
        titleColor: "#111",
        bodyColor: "#111",
        backgroundColor: "rgba(255,255,255,.92)",
        borderColor: "rgba(0,0,0,.1)",
        borderWidth: 1,
      },
    },
  } as const;

  return (
    <div className="cap-root">
      <section className="cap-card">
        {/* 헤더 */}
        <header className="cap-header">
          <h1>게임 기록 상세</h1>
          <p className="cap-sub">
            범인 추리 : <b>{culpritName || "미입력"}</b>
          </p>
          <p className={`cap-result ${correct ? "correct" : "wrong"}`}>
            {correct ? "정답입니다" : "틀렸습니다 ❌"}
          </p>
        </header>

        {/* KPI */}
        <section className="cap-kpis">
          <div className="cap-kpi">
            <div className="cap-kpi-label">평균 점수</div>
            <div className="cap-kpi-value">{avgScore}</div>
          </div>
          <div className={`cap-kpi ${correct ? "pos" : "neg"}`}>
            <div className="cap-kpi-label">정답 여부</div>
            <div className="cap-kpi-value">{correct ? "정답" : "오답"}</div>
          </div>
        </section>

        {/* 차트 */}
        <div className="cap-charts-row">
          <article className="cap-chart-tile">
            <h2>능력치 분석</h2>
            <div className="cap-chart-area">
              <Radar data={data} options={options} />
            </div>
          </article>
        </div>

        {/* 제출 답변 요약 */}
        <section className="cap-feedback">
          <h3>제출한 답변</h3>
          <ul>
            <li>
              <b>언제:</b> {answerJson?.when || "(미입력)"}
            </li>
            <li>
              <b>어떻게:</b> {answerJson?.how || "(미입력)"}
            </li>
            <li>
              <b>왜:</b> {answerJson?.why || "(미입력)"}
            </li>
          </ul>
        </section>

        {/* 버튼 */}
        <div className="cap-actions">
          <button className="cap-exit-btn" onClick={() => navigate(-1)}>
            목록으로 돌아가기
          </button>
        </div>
      </section>
    </div>
  );
}
